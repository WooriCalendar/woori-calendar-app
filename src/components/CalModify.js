...
const CalModify = (props) => {
  ...
  useEffect(() => {
    call("/calendar/" + calNo, "GET", null).then((response) => {
      ...
      document.querySelector(".color .notranslate").innerHTML =
  ...
  const onTimeZoneChange = async (e) => {
    await axios
      ...
      .then((response) => {
        setCalendar({
          ...calendar,
          timeZone: e.target.value + " (utc " + response.data.utc_offset + ")",
        });
  ...
  const onGradeChange = () => {
    call("/share", "PUT", shareMember).then((response) => {
            ...
            >
              ...
              <div style={{ marginLeft: "auto" }}>
                ...
                <Select
                  ...
                  disabled={
                    share[0]
                      ? share[0].grade !== 2 || member.email === share[0].email
                      : null
                  }
                  onChange={(e) => {
