...
import {call, findemail, forgotEmail, signupemail, signuppassword} from "../service/ApiService";
            ...
            if(resp.subemail) {
                setTimeout(() => {
                    // findemail({email: subemail}).then((resp) => {
                        ...
                        // console.log(code);
                        setLoding(true);
                        document.getElementById('emailCheck').innerText = "please check your email.";
                    // });
                }, 100);
            }else{
                //없는 보조이메일
                ...
                <Grid item xs={12}>
                    <Slider ref={sliderRef} {...settings} style={{height: "10%"}}>
                        <div>
                            ...
                            {loding ? (
                                <Button fullWidth variant="contained" color="primary" id={"nextBtn"} onClick={handleEmail} disabled={buttonDisabled} style={{marginTop : "5%"}}>
                                    Send
                                </Button>
                            ) : (
